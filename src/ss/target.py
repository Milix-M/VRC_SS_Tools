...
def get_target_ss(path: str) -> list:
    ...
    # old naming example: VRChat_7680x4320_2022-10-06_22-50-52.350.png
    pattern_old = r'VRChat_\d{4}x\d{4}_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}.\d{3}.png'
    ...
    for dir in path:
        for root, dirs, files in os.walk(dir):
            for file_name in files:
                #最新形式と古い命名形式両方チェックしてssリストにappend
                if re.match(pattern, file_name) or re.match(pattern_old, file_name):
